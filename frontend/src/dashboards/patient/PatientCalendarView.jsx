...
// calendar view component for patient dashboard
function PatientCalendarView({ appointments, downloadPrescription }) {
    ...
    <div className="calendar-container mt-4 z-0 relative">
      ...
      <FullCalendar
        ...
        eventClick={(info) => {
          ...
          // check if prescription exists to prompt download
          if (appt.hasPrescription) {
            if (
              window.confirm(
                `Appointment with Dr. ${appt.doctorName} was ${appt.status}. Do you want to download the prescription?`
              )
            ) {
